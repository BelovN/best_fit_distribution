...
def get_best_fit_distribution(data, bins=200, ax=None):
...
def load_data():
    ...
    for file_name in file_names:
        data[file_name.replace('.csv', '')] = pd.read_csv(os.path.join(DATA_DIR, file_name),
                                                          sep=',', encoding='utf-8',
                                                          parse_dates=['Date_UTC'], dayfirst=True,
                                                          index_col='Date_UTC')
    return data
...
def main():
    ...
    best_fit_name, best_fit_params = get_best_fit_distribution(data, 200, ax)
